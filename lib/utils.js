...
exports.xmlizeMessage = function xmlizeMessage(options, message) {
  ...
  } else if (options.json === false || (this._settings.json === false && options.json !== true)) {
...
exports.decodeMessage = function decodeMessage(options, message) {
  if (options.json === true || (this._settings.json === true && options.json !== false)) {
    try {
      // decode a likely base64 to string (to support VS2013 queue browser which is doing base64 by default, why?)
      if (message.messageText[0] !== '{' || message.messageText[0] !== '[') {
